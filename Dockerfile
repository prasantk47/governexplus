...
# Install cython first (required for pyrfc), then pyrfc with SAP SDK
RUN pip install --no-cache-dir cython wheel setuptools && \
    SAPNWRFC_HOME=/usr/local/sap/nwrfcsdk pip install --no-cache-dir pyrfc || echo "pyrfc build failed" && \
